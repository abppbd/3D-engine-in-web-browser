...
function pointOffScreen(fixedPoint, movablePoint, edgeXdist=4, edgeYdist=2){
  ...
  let nexX = objectClone(movablePoint[0])
  let newY = objectClone(movablePoint[1])

  if (!(-hCanvas_w > nexX > hCanvas_w & -hCanvas_h > newY > hCanvas_h)){
    // If the point to move is on Screen.

    if (movablePoint[0] < 0){
      
      edgeXdist *= movablePoint[0] < 0 ? 1 : -1
      // Movable point is on the screen's left (use left screen edge).
    }
    if (movablePoint[1] < 0){
      // Movable point is on the top of the screen (use ottom screen edge).
      edgeYdist *= movablePoint[1] < 0 ? 1 : -1
      // Movable point is on the top of the screen (use ottom screen edge).
    }
...
function renderEdge(vert, edges){
  ...
  for (edgeIdx in edges){
    ...
    if (toDraw1 || toDraw2){
      ...
      let v1Proj = perspectiveProj(v1)
      let v2Proj = perspectiveProj(v2, true, true)
      ...
      if (toDraw1 != toDraw2){
        ...
        hidenProj = pointOffScreen(frontProj, hidenProj, hCanvas_w, hCanvas_h)
