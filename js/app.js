...
var V_FOV = 90       // Horizontal Field Of View.
...
function spherePerspectiveProj(point){
  ...
  |/   |
  +-------> +Y
  angle a = atan(y/x)  # h_angle
  Get proportion of H_FOV taken by the angle a (factor x).
  fx = (2*a) / H_FOV
  fx = (2*atan(y/x)) / V_FOV
  Get screen x poition (Sx) from screen width (Sw):
  Sx = Sw * fx
  Sx = (Sw*V_FOV) / (2*atan(y/x))

  Side view, get Screen y.
  +Z
  ^
  |----* P(x, z)
  |   /|
  ...
  |/b  |
  +-------> +X
  angle b = atan(z/x)  # v_angle
  Get proportion of V_FOV taken by the angle b (factor y).
  fy = (2*b) / V_FOV
  fy = (2*atan(z/x)) / V_FOV
  Get screen y poition (Sy) from screen height (Sh):
  Sy = Sh * fy
  Sy = (Sh*V_FOV) / (2*atan(z/x))
  ...
  */
  ...
  //// Get screen position.
  //let Sx = Sw * ((2*Math.atan(y/x)) / rH_FOV)
  //let Sy = Sh * ((2*Math.atan(z/x)) / rV_FOV)
...
function renderEdge(vert, edges, debug=false){
  // "vert" is the list of vertices and "edges" is the edges_keys associated.

  for (edgeIdx in edges){
    // Loop over every edge.

    let v1 = vert[edges[edgeIdx]["edge"][0]]["point"]
    let v2 = vert[edges[edgeIdx]["edge"][1]]["point"]
    // Get the edge's vertices' coords.
    
    v1Proj = spherePerspectiveProj(v1)
    v2Proj = spherePerspectiveProj(v2)
    
    if (-hCanvas_w <= v1Proj[0] <= hCanvas_w &&
        -hCanvas_h <= v1Proj[1] <= hCanvas_h &&
        -hCanvas_w <= v2Proj[0] <= hCanvas_w &&
        -hCanvas_h <= v2Proj[1] <= hCanvas_h){
    
      drawLine(v1Proj[0], v1Proj[1], v2Proj[0], v2Proj[1], true,
        edges[edgeIdx]["color"], debug=false)
    }

    let toDraw1 = isInFront(v1)
    let toDraw2 = isInFront(v2)
...
function loadJSON(){
  ...
  [{"Id": 3, "name": "line", "mode": "ep", "isRendered": false, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
   ...
   {"Id": 3, "name": "test", "mode": "pe", "isRendered": true, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
