...
var rotAlpha = 10 // Player alpha angle step.
var rotBeta = 10  // Player beta angle step.

var H_FOV = 90       // Horizontal Field Of View.
var screenDist = 200 //FOVtoDist(H_FOV, canvas_w) // Dist screen to player/cam.
var camPlaneClip = 0.1 // Min dist from cam for rendering.
...
function pointOffScreen(fixedPoint, movablePoint, edgeXdist=4, edgeYdist=2, debug=false){
  ...
  console.log("")
  console.log("fix & mov:", fixedPoint, movablePoint)

  if (!(-hCanvas_w > newX > hCanvas_w & -hCanvas_h > newY > hCanvas_h)){
    // If the point to move is on Screen.

    if (movablePoint[0] < 0){
      // Movable point is on the screen's left (use left screen edge).
      edgeXdist *= -1
    }
    if (movablePoint[1] < 0){
      // Movable point is on the top of the screen (use bottom screen edge).
      edgeYdist *= -1
    }

    console.log("edgeXdist:", edgeXdist, "| edgeYdist:", edgeYdist)


    let fToEx = edgeXdist - fixedPoint[0]
    let fToEy = edgeYdist - fixedPoint[0]
    // Get edges pos rel to the fixed point.
    console.log("Fixed to edge: X:", fToEx, "| Y:", fToEy)

    let xDif = movablePoint[0] - fixedPoint[0]
    let yDif = movablePoint[1] - fixedPoint[1]
    // Get Dist from movable to fixed point.
    console.log("mov/fix dif: X:", xDif, "| Y:", yDif)

    let kX = fToEx/xDif
    let kY = fToEy/yDif
    kX = Math.round(Math.abs(kX + 0.5))
    kY = Math.round(Math.abs(kY + 0.5))
    // Get x & y factors (0<=k<=1) then round away from zero (go off screen).
    console.log("Factors: X:", kX, "| Y:", kY)

    let k = Math.min(Math.abs(kX), Math.abs(kY))
    // Get factor closest to zero.
    console.log("chosen factor:", k)

    newX = (k * xDif)
    newY = (k * yDif)
    console.log("Multiplied coords:", [newX, newY])
    newX = Math.abs(newX) * Math.sign(movablePoint[0]) + fixedPoint[0]
    newY = Math.abs(newY) * Math.sign(movablePoint[1]) + fixedPoint[1]
...
function loadJSON(){
  let geom = 
  [{"Id": 3, "name": "test", "mode": "e", "isRendered": true, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
    "points":[{"point":[0, 0, 0], "color": "#000000"}, {"point":[0, 3, 0], "color": "#000000"}],
    "edges": [{"edge": [0,1], "color": "#000000"}],
    "faces": []
   },
   {"Id": 3, "name": "test", "mode": "pe", "isRendered": false, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
