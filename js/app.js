...
function drawInfinityAndNaN(x, y){
  ...
  if (isFinite(x) & !isFinite(y)){
    ...
    console.log("b", dist, Math.sign(y))
...
function drawLine(x1, y1, x2, y2, center=false, color="#000000", debug=false){
  ...
  if(debug){console.log("p1:", [x1, y1], "p2:", [x2, y2], isFinite(x1), isFinite(y1), isFinite(x2), isFinite(y2))}
...
function pointOffScreen(fixedPoint, movablePoint, edgeXdist=4, edgeYdist=2, debug=false){
  ...
  if (debug){console.log(movablePoint, "newX:", newX, " | newY", newY, )}
  if (!(-hCanvas_w > newX > hCanvas_w & -hCanvas_h > newY > hCanvas_h)){
    ...
    /*if (!(Number.isFinite(newX) || Number.isFinite(newY))){
      console.log("shit", newX, newY)
    }*/
  }
  if (debug){console.log("output:", [newX, newY])}
  return [newX, newY]
}
...
function renderEdge(vert, edges){
  ...
  for (edgeIdx in edges){
    ...
    if (toDraw1 || toDraw2){
      ...
      if (edgeIdx == 0){console.log("edge proj", v1Proj, v1, v2Proj, v2)}
...
function loadJSON(){
  let geom = 
  [{"Id": 0, "name": "test", "mode": "pe", "isRendered": true, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
