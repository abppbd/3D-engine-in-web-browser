...
function drawPolygon(points, color="#8EBA63"){
  let pointsOnCanvas = points.map(
    function (point, idx){
      point = [point[0] + hCanvas_w, point[1] + hCanvas_h]
      // Make point rel to canvas cuz (0,0) != canvas center

      point[1] = canvas_h - point[1]
      // Flip it cuz Y canvas is upsidedown.

      point = drawInfinityAndNaN(point[0], point[1])
      // Adapt for +/-Inf & NaN coords
      console.log("drawPoly", point)
...
function renderPolygon(vert, faces){
  ...
  for (orderIdx in order){
    ...
    let faceIdx = order[orderIdx]
    // Get current face idx.

    let faceVertIdx = faces[faceIdx]["face"]
    // Get vert idx of current face.

    let faceVert = faceVertIdx.map(
        function (vertIdx, idx){
          return vert[vertIdx]["point"]
    })
    // Get vert pos of current face.

    if (faceVert.some(e => isInFront(e))){
      // If any of the face's vertices are in front of the ZY plane.

      faceVertProj = faceVert.map(
        function (vertPos, idx){
  ...
  for (const orderIdx in order) {
    ...
    if (shape["isRendered"] == true){
      ...
      if (renderF & geometry[shapeIdx]["mode"].includes("f")){
        ...
        renderPolygon(relVert, shape["faces"])
      }
    }
  }
}
...
function loadJSON(){
  let geom = [
  {
    "Id": 3, "name": "line", "mode": "e", "isRendered": false, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
