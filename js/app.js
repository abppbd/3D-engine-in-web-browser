...
const hCanvas_h = canvas_h / 2
const hCanvas_w = canvas_w / 2
...
var moveX = 1 // forward/back.
var moveY = 1 // right/left.
var moveZ = 1 // up/down.

var p_alpha = 0 // Player alpha angle (+: right / -: left).
var p_beta = 0  // Player beta angle (+: up / -: down).

var rotAlpha = 5 // Player alpha angle step (right/left).
var rotBeta = 10  // Player beta angle step (up/down).

var H_FOV = 90      // Horizontal Field Of View.
let screen_ratio = canvas_w/canvas_h
var V_FOV = H_FOV/screen_ratio       // Horizontal Field Of View.
var screenDist = 200 //FOVtoDist(H_FOV, canvas_w) // Dist screen to player/cam.
var camPlaneClip = 0.1 // Min dist away from cam for rendering.
...
// Return some int coordinates for drawing with NaN & Infinity.
function drawInfinityAndNaN(x, y){
  let newX = x
  let newY = y
  
  x = x || 0
  y = y || 0
  // Convert NaN (and false, null, undefined, "") to 0.

  let dist = Math.max(canvas_h, canvas_w)
  // Use the longest side to ensures the drawing goes off screen.

  if(!isFinite(x) & isFinite(y)){
    // If only the X component is +/-Inf (make a horizontal line).
    newX = dist * Math.sign(x)
    newY = 0
    return [newX, newY]
  }

  if (isFinite(x) & !isFinite(y)){
    // If only the Y component is +/-Inf (make a vertical line).
    newY = dist * Math.sign(y)
    newX = 0
    return  [newX, newY]
  }
  
  if (!(isFinite(x) & isFinite(y))){
    // If both X & Y are +/-Inf (make a diagonal line)
    newX = dist * Math.sign(x)
    newY = dist * Math.sign(y)
    return  [newX, newY]
  }
...
function drawLine(x1, y1, x2, y2, center=false, color="#000000", debug=false){
  ...
  if (!(isFinite(x2) & isFinite(y2))){
    [x2, y2] = drawInfinityAndNaN(x2, y2)
  }
  // Draw +/-Inf & NaN
...
function drawNGon(vert, color = "#E62169"){
  ctx.beginPath()  // Start polygon drawing.

  // Plot 1st point (move to).
  let p1 = vert[0]
  p1[0] += hCanvas_w
  p1[1] += hCanvas_h
  p1[1] = canvas_h - p1[1]
  p1 = p1.map(function (v){return Math.round(v)})
  ctx.moveTo(p1[0], p1[1])

  // Plot the other points (line to).
  for (let idx=1; idx<vert.length; idx++){

    // Point rel to canvas; (0,0) != canvas center.
    let px = vert[idx][0] + hCanvas_w
    let py = vert[idx][1] + hCanvas_h

    // +Y is cnvas bottom.
    py = canvas_h - py

    // Canvas ploting requires Int.
    px = Math.round(px)
    py = Math.round(py)
  ...
  ctx.closePath()          // Stop polygon drawing.
  ctx.fillStyle = color
  ctx.fill()
  ctx.lineWidth = 0        // No strokes.
  ctx.strokeStyle = color  // Seamless.
  ctx.stroke()             // Draw.
}
...
function objectRenderOrdering(objs){
  ...
  for (objIdx in objs){
    ...
    let objPos = objs[objIdx]["position"] // Get obj position.
    let distToCam = distPointsPow2([cam_x, cam_y, cam_z], objPos)
    // Get the dist from cam to obj pos.
...
// Get element-wise avreage of the sublists in arr.
function getSubArrAvg(arr){
  // Every elements in arr must have THE SAME LEN.
  // Ex: arr = [[1,2,7],[5,3,40]] -> avrgs = [3, 2.5, 23.5]

  let avgs = []
  for (itemIdx in arr[0]){
    // Loop over every item index in the sublists.
 
    let sum = 0
    for (arrIdx in arr){
      sum += arr[arrIdx][itemIdx]
    }
    // Sum all elements w/ same idx in sublist.

    avgs.push(sum/arr.length)
    // Add aveage.
  }
...
// Gives the face rendering order of an object (works w/ n-gons).
// Painter's algorithm.
function faceRenderingOrder(faces, vert){
  let facesDist = []

  for (faceIdx in faces){
    let faceVert = []
    for (i in faces[faceIdx]["face"]){
      faceVert.push(vert[faces[faceIdx]["face"][i]]["point"])}
    // Get face's vertices.
    let faceAvgPos = getSubArrAvg(faceVert)
    // Get face's center pos.

    let distToCam = distPointsPow2([0, 0, 0], faceAvgPos)
    // The points are rel to the cam so the cam is (0, 0, 0).

    facesDist.push([distToCam, parseInt(faceIdx)])
  }

  facesDist = facesDist.sort(function(a, b){return b[0]-a[0]})
  // Sort in ascending order.

  let idxOrder = facesDist.map(
    function (item, idx){
      return item[1]
    })
  // Only get Idx of faces.
...
function moveCam(mF, mB, mR, mL, mU, mD){
  // Variables as m* are boolean, multiplying by 1 or 0 the values to offset
  // the cam.
  ...
  if (mF || mB || mR || mL){
    ...
    // Get x & y component when going front/back according to player angle.
...
document.addEventListener("keydown", function(e) {
  ...
  // Show player angle.
  output(["Rot: alpha", p_alpha, " | beta", p_beta], 2)
  output(["Pos: X", cam_x, " | Y", cam_y, "| Z", cam_z], 3)

  // Show keyPressed's index
  output(["Pressed key index:", e.which], 4)
...
function perspectiveProj(point, conpensateSign=true, debug=false){
  ...
  // Distance from screen's center (projection coords).
...
// Project the points on a sphere around the cam (using angles).
function spherePerspectiveProj(point){
  ...
  let rH_FOV = degToRad(H_FOV)
  let rV_FOV = degToRad(V_FOV)

  // We want the 2 angles describing a point relative to the camera orientation.
  
  /* The 1st angle is on the XY plane (ground) and represents the x screen
  coordinates (as viewed from above, +z -> -z):
    Top down view, get Screen x.
    +X
    ^
    |----* P(y, x)
    |   /|
    ...
    |/   |
    +-------> +Y
    angle a = atan2(y, x)  # h_angle

  Get proportion of horizontalFOV taken by the angle a as screen x:
    Size: | Angle:
    ------+-------
    Sw/2  | hFOV/2    <=>     Sx = (a*(Sw/2)) / (hFOV/2)
    ------+-------
    Sx    | a

  As one formula:
    Sx = (atan2(y, x)*(Sw/2)) / (hFOV/2)
  */
  let Sx = (Math.atan2(y, x) * Sw/2) / (rH_FOV/2)
  
  
  /* The 2nd angle is made between the origin-point segment and the XY plane.
  The 2nd angle depends on the 1st, thus a simple XZ plane projection is not
  possible.
  
  Get the distance between the point and the origin on the XY plane:
    Pxy = sqrt(x^2 + y^2)
    
  Get the angle:
    angle b = atan2(z, Pxy)
  
  Get proportion of verticalFOV taken by the angle b as screen y:
    Size: | Angle:
    ------+-------
    Sh/2  | vFOV/2    <=>     Sy = (b*(Sh/2)) / (vFOV/2)
    ------+------- 
    Sy    | b

  As one formula:
    Sy = (atan2(z, Pxy)*(Sh/2)) / (vFOV/2) <=>
    Sy = (atan2(z, sqrt(x^2 + y^2))*(Sh/2)) / (vFOV/2)
  */
  
  let b = Math.atan2(z, Math.hypot(x, y))

  let Sy = (Math.atan2(z, Math.sqrt(x^2 + y^2))*(Sh/2)*2) / (rV_FOV/2)
  Sy = (b * (Sh)) / (rV_FOV)
  /*console.log("point:", point)
  console.log(Math.atan2(z, Math.sqrt(x^2 + y^2)))
  console.log("") */
...
// Send a Point off screen while maintaining the segment direction.
function pointOffScreen(fixedPoint, movablePoint, edgeXdist=4, edgeYdist=2, debug=false){
  // Make the point go off screen by multiplying x & y by a factor k > 1.
  // An edge' dir is maintained cuz x & y are mutiplyed by the same value.

  let newX = objectClone(movablePoint[0])
  let newY = objectClone(movablePoint[1])

  if (!(-hCanvas_w > newX > hCanvas_w & -hCanvas_h > newY > hCanvas_h)){
    // If the point to move is on Screen.
    if (fixedPoint[0] > movablePoint[0]){
      // Mov to the left of fix, use left (-) edge.
      edgeXdist = -edgeXdist
    }
    if (fixedPoint[1] > movablePoint[1]){
      // Mov under fix, use bottom (-) edge.
      edgeYdist = -edgeYdist
    }
...
function inFOV(point){ // point = [x, y]
  ...
  return (Math.abs(point[0]) < hCanvas_w && Math.abs(point[1]) < hCanvas_h)
...
function renderVertex(vert){
  for(vertIdx in vert){
    ...
    // Project point onto the screen.
    screenProj = spherePerspectiveProj(vertCo)
...
function renderEdge(vertRel, vertProj, edges, debug=false){
  // "vert" is the list of vertices and "edges" is the edges_keys associated.

  for (edgeIdx in edges){
    // Loop over every edge.

    let v1Proj = vertProj[edges[edgeIdx]["edge"][0]]["point"]
    let v2Proj = vertProj[edges[edgeIdx]["edge"][1]]["point"]

    // If the edge can be seen, draw it.
    if (inFOV(v1Proj) || inFOV(v2Proj)){
...
function renderTriangle(vert, faces){
  // "vert" is the list of vertices and "faces" is the faces' verices' indexes.
  
  let order = faceRenderingOrder(faces, vert)

  for (orderIdx in order){
    // Loop over every face.

    faceIdx = order[orderIdx]

    v1 = vert[faces[faceIdx]["face"][0]]["point"]
    v2 = vert[faces[faceIdx]["face"][1]]["point"]
    v3 = vert[faces[faceIdx]["face"][2]]["point"]
    // Get the face's vertices' coords.
    
    v1Proj = spherePerspectiveProj(v1)
    v2Proj = spherePerspectiveProj(v2)
    v3Proj = spherePerspectiveProj(v3)

    if (inFOV(v1Proj) || inFOV(v2Proj) || inFOV(v3Proj)){
      drawTriangle(v1Proj, v2Proj, v3Proj, faces[faceIdx]["color"])
    }
    ...
    /* if (isInFront(v1) || isInFront(v2) || isInFront(v3)){
      ...
      // Project the points onto the screen (cuz they're already rel to cam).
...
function renderNGon(vertRel, vertProj, faces){
  // "vert" is the list of vertices and "faces" is the faces' verices' indexes.
  
  let faceOrder = faceRenderingOrder(faces, vertRel)

  // Loop over every face.
  for (orderIdx in faceOrder){

    faceIdx = faceOrder[orderIdx]

    // Get face's projected vertices.
    let faceVert = faces[faceIdx]["face"].map(
      function (vIdx, idx){
        return vertProj[vIdx]["point"]
      }
    )

    // if any of the face's points are visible draw face.
    if (faceVert.some(inFOV)){
      drawNGon(objectClone(faceVert), faces[faceIdx]["color"])
      // ObjectClone so the rendering transform doesn't affect the original 
      // vertices projection.
    }
  }
}
...
function rendering(geometry, renderV=true, renderE=true, renderF=true){
  ...
  for (const orderIdx in order) {
    ...
    if (shape["isRendered"] == true){
      ...
      // Project all point on screen at once to aoid redundant computation.
      let projVert = relVert.map(
        function (vert, idx){
          let v = {
            "point" : spherePerspectiveProj(vert["point"]),
            "color" : vert["color"]
          }
          return v
        }
      ...
      if (renderV & geometry[shapeIdx]["mode"].includes("p")){
        ...
        renderVertex(projVert)
      }

      if (renderE & geometry[shapeIdx]["mode"].includes("e")){
        // If edge and shape edge mode are enabled, render edges.
        renderEdge(relVert, projVert, shape["edges"])
      }

      if (renderF & geometry[shapeIdx]["mode"].includes("f")){
        // If face and shape face mode are enabled, render faces.
        renderNGon(relVert, projVert, shape["faces"])
        //renderTriangle(relVert, shape["faces"])
      }
    }
  }
}
...
function loadJSON(){
  ...
  [
    {
      "Id": 3, "name": "line", "mode": "ep", "isRendered": false, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
      "points":[{"point":[0, 0, 0], "color": "#000000"}, {"point":[0, 3, 0], "color": "#000000"}],
      "edges": [{"edge": [0,1], "color": "#000000"}],
      "faces": []
    },
    {
      "Id": 3, "name": "triangles", "mode": "fe", "isRendered": false, "position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0],
